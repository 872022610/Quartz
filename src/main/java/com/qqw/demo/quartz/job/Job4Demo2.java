...
@Component
public class Job4Demo2 extends QuartzJobBean {
    ...
    public void executeInternal(JobExecutionContext jobExecutionContext){
        ...
        System.out.println("开始执行job2");

        //获取外部参数
        JobDataMap jobDataMap = jobExecutionContext.getMergedJobDataMap();
        System.out.println("job2参数列表" + JsonPluginsUtil.mapToString(jobDataMap));

        DemoTable insertDemoTable = new DemoTable();
        insertDemoTable.setCreateTime(new Date());
        insertDemoTable.setJobClassName("Job4Demo2");
